...
export default function StockOutwardInvoice() {
  ...
  const fetchInvoice = async () => {
    try {
      const response = await fetch(`/api/stock/outward/${params.id}`)
      if (response.ok) {
  ...
  const handleDownload = async () => {
    try {
      const response = await fetch(`/api/invoice/outward/${params.id}/download`)
      if (response.ok) {
        const blob = await response.blob()
        const url = window.URL.createObjectURL(blob)
        const a = document.createElement("a")
        a.href = url
        a.download = `sales-invoice-${params.id}.html`
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6 print:hidden">
          <Link href="/stock/outward">
            <Button variant="outline">
              <ArrowLeft className="mr-2 h-4 w-4" />
              Back to Stock Outward
            </Button>
          </Link>
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
              ...
              <div className="text-right">
                <div className="text-sm">
                  ...
                  <p>
                    <strong>Type:</strong> Sales Invoice
                  </p>
                </div>
              </div>
            </div>
