...
export default function StockInwardInvoice() {
  ...
  const fetchInvoice = async () => {
    try {
      const response = await fetch(`/api/stock/inward/${params.id}`)
      if (response.ok) {
  ...
  const handleDownload = async () => {
    try {
      const response = await fetch(`/api/invoice/inward/${params.id}/download`)
      if (response.ok) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <Card className="print:shadow-none print:border-none">
          <CardHeader className="text-center border-b">
            ...
            <p className="text-gray-600">Purchase Invoice - Stock Inward</p>
          </CardHeader>
