...
export default function StockOutward() {
  ...
  const fetchParties = async () => {
    try {
      const response = await fetch("/api/parties?type=sales")
      if (response.ok) {
  ...
  const fetchNextInvoiceNumber = async () => {
    try {
      const response = await fetch("/api/invoice/next-number?type=sales")
      if (response.ok) {
        const data = await response.json()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("/api/stock/outward", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "Stock outward added successfully",
        })
  ...
  return (
    <Layout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold">Stock Outward</h1>
          <p className="text-gray-600">Add outgoing stock with sales invoice details</p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Add Stock Outward</CardTitle>
            <CardDescription>Enter sales invoice details and items</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="party">Sales Party</Label>
                  <Select
                    value={formData.partyId}
                    onValueChange={(value) => setFormData((prev) => ({ ...prev, partyId: value }))}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Select sales party" />
                    </SelectTrigger>
                    <SelectContent>
                      {parties.map((party) => (
                ...
                <div className="space-y-2">
                  <Label htmlFor="invoiceNumber">Sales Invoice Number</Label>
                  <Input id="invoiceNumber" value={formData.invoiceNumber} readOnly className="bg-gray-50" />
              ...
              <Button type="submit" className="w-full" disabled={loading}>
                {loading ? "Please wait..." : "Add Stock Outward"}
              </Button>
            </form>
          </CardContent>
        </Card>
