...
export async function PUT(request, { params }) {
  try {
    ...
    const result = await stockOutward.updateOne(
      { _id: new ObjectId(id) },
      {
        $set: {
          ...data,
    ...
    console.error("Error updating stock outward:", error)
...
export async function DELETE(request, { params }) {
  try {
    ...
    const result = await stockOutward.deleteOne({ _id: new ObjectId(id) })
    await client.close()
